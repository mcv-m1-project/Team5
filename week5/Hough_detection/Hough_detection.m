...
for n = 1:size(files, 1)
    ...
    for n_BBox = 1:size(windowCandidates,1)
        ...
        if ~isempty(Peaks)
            lines = houghlines(Window, theta, rho, Peaks, 'FillGap', 8, 'MinLength', Thresholds.min_D_rect*3/4);
            ...
            for i = 1:size(lines, 1)
                ...
                angle = lines(i).theta + 90;
                
                if abs(angle) < Thresholds.T_theta_h
                    
                    idx_h = idx_h + 1;
                    horizontal(idx_h) = i;
                    
                elseif abs(abs(angle) - 90) < Thresholds.T_theta_v
                    
                    idx_v = idx_v + 1;
                    vertical(idx_v) = i;
                    
                elseif abs(abs(angle) - 60) < Thresholds.T_theta_i
            ...
            if length(horizontal) >= 1
                if length(inclined) >= 2
                    angle_inclined = lines(inclined).theta - 90;
